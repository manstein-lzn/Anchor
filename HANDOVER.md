...
本次交接核对时间：`2026-09-11 15:30 CST`；Verifier 收尾、执行策略、内容面、agent surface 与深度调研会话完成下述验收并更新本文。

## 上下文管理立项：先造尺子（2026-09-11，ADR-044）

产品方向已明确：**上下文管理是 Anchor 最终的核心**，其余部分是为了让它可做而存在。
本段记录为什么第一步不是上下文管理本身。
...
模型自己决定检索什么、读哪篇、综合什么。于是改了策略之后结果变好了，**你分不清是
策略起作用还是模型这次发挥好**。改进无法归因 → 无法测量 → 无法开发。

而这件事早已声明：`PRODUCT_VISION.md` 的 I9 要求 R0/R1，并明确它们是唯一的保证级别。
**未实现。**

### 决定（ADR-044）

录制每次模型调用的请求与响应，使一次运行可确定性回放。三条约束：

1. **录制是投影，不是状态**（I2）。永不参与恢复。
2. **回放按结构位置匹配**（node, attempt, sequence），不按 prompt 哈希——目的正是改变 prompt。
3. **分歧显式失败**并定位到具体调用。静默继续会让"回放成功"变成假信号。

**包装层在 PydanticAI 的 `Model`，不在 Anchor 的 `ModelGateway`。** 这是核实后的关键判断：
一次 `generate_with_tools()` 等于一整个 agent 轮次（PydanticAI 在 `agent.run()` 内部跑完
工具循环），在 gateway 层录制会丢失循环内部不断增长的上下文——而那正是需要观察的对象。
`WrapperModel` 是库自身的扩展点，`durable_exec` 的 dbos/prefect/temporal 三个后端都用它。

### 诚实边界（已写入 ADR 与文档）

**回放不能用于 A/B 一个会改变 prompt 的上下文策略。** prompt 变了，模型必须真实重新调用；
提供旧响应测不出任何东西。本立项只买到**可观测性与确定性复现**。

降低迭代成本需要的是**节点级测试台**（冻结输入快照 + 单独运行一个节点，把 31 分钟压到
分钟级）——今天不存在（`client.run_nodes` 是只读），且比步骤 1–3 加起来还大。**另行决定。**

### 未验证的阴性结论（已核实，勿当成已知）

- 全仓无任何模型调用录制/回放代码
- 无单独运行一个节点的能力
- `WrapperModel` 存在且被 durable_exec 三个后端使用

### 产物

- `docs/RECORDING_AND_REPLAY.md`——完整方案、体积估算（一次 campaign 8.9 MB 原始 /
  2.9 MB 去重后，占当前 212 MB 工件库约 1.4%）、风险与禁止事项、验收标准、实施顺序
- `DECISIONS.md` ADR-044

## 深度调研的成本、收敛与交付物（2026-09-10 → 09-11，ADR-042 / ADR-043）

一次真实运行暴露的问题不是模型不行，而是**我们把它放在了一个错误的循环里**。本段记录修复。

### 第一次交付物被判定为"低质量"

最初那版报告正文里 57% 的段落带审计语言、116 个证据级标签、35.5% 篇幅是文献器械。
它不是报告，是审计台账。系统精确执行了当时写的契约——契约错了。

修：拆角色（ADR-040）。`gather` 只出证据账本不出正文，`write` 只写正文不碰工具，
`review` 同时评实质与表达并给出 `target`，`check` 按 target 路由。执行结果：
同题 7.2 分钟 vs 25.8，38KB vs 91KB，正文审计词 0。

### 章节骨架被判定为"自己编的"

原骨架（Methods / Literature Review / Limitations）是 IMRaD 尾巴缝在综述正文上，不对应任何真实论文。

修：用项目自己的 `scholarly.read` 抓了四篇论文目录（`1801.04405` ACM CSUR、`1304.1002` SLR、
`1808.04836` survey study、`2002.12418` systems），归纳出三条硬规律并照抄（ADR-041）。

### 钱烧得太快

用户给出的实测：¥65.34 / 1,707 请求 / 242,479,807 tokens。根因四处，均已修：

1. **`RETRY_CONTEXT_CHAR_BUDGET = 80_000`**：每轮把累积的历史证据重放进系统提示，
   注入**每一次模型调用**且每轮都变→每轮一次全额计费的缓存未命中。现在只在真正重试时重放，
   预算降到 12,000；前一次尝试已完成的新轮次不重放。
2. **截断配置反了**：`scholarly.citations` 20 条 × 1500 字符 = 单次 38K 字符。
   现在 citations 5×400、search 5×500、read 1×2000。
3. **模型轮次太高**：读一篇 ≈ 1.7 个模型轮次。新增 `scholarly.read_many`（一批最多 8 篇，
   并行抓取但仍逐源限速），arXiv PDF 直接取 HTML 渲染（之前先试 PDF 再试 HTML，白花一倍限速）。
   轮次降到 ~0.17/篇。
4. **没有记账**：新增 `model.usage` 事件与 `GET /api/runs/{id}/usage`，
   分别报告 gross / cached / billed input。只看 gross 会高估账单数倍——实测工具循环的
   前缀缓存命中率 81–90%。

### 一条 never-ending 的循环

一次 campaign 跑了 20 轮不收敛，一次修订循环跑了 18 轮修同一个缺陷。四处结构缺陷：

1. **无终止判据**（ADR-042）：现在研究在"模型自报饱和 / 某轮零新增 / 连续 3 轮新增 < 语料 5%"时停。
2. **坏引用被路由到采集器**：账本越长论文不改，同一引用反复回来。现在按"证据缺陷 vs 写作缺陷"
   分流；不可修复的引用由作者丢弃。
3. **引用编号漂移**：`drop_unverifiable` 剔除来源后重编号，作者修订的旧稿仍引旧号→
   稿件指向错误来源，18 次修订全部无效。现在编号不变，留下空洞。
4. **门禁与验证器两套来源策略**：门禁只看 `evidence_ref` 是成功操作，验证器还要求 id 在产物里、
   读取为同一篇。于是来源可以被**准入却不可引用**，作者拿到一份无法满足的契约。
   现在两者共用 `runtime/evidence.py`。

### 评审在违反自己的标准（ADR-043）

一篇论文改了 9 轮。评审的指令写着 "pass requires no unresolved major issues"，而它在三轮里
报出 **0 个 major**，仍给 `revise`（6、6、4 条 minor）。细心的评审总能找到 minor，所以循环永远不结束。

修：批准不再依赖模型自我克制。**只报 minor 且确定性检查通过 → 批准**，并记录 `approved_with`。
这就是 "accept with minor revisions"——编辑做出的、让细心评审无法扣住一篇已达标的论文的决定。

### 可靠性缺陷

| 缺陷 | 表现 | 修复 |
|---|---|---|
| 心跳失败静默取消 | lease 泄漏、节点永久 `running`、run 卡在它后面 | 记日志、容忍 2 次连续失败、第 3 次**显式失败节点并释放 lease** |
| dev 服务无日志 | 卡死无堆栈可查 | `start-services.sh` 把 stdout/stderr 落到 `.local/logs/` |
| JSON 前有说明文字 | 评审输出 3070 字符、JSON 完整，仍判 `agent_output_invalid` | `runtime/json_output.py` 容错提取 |
| 同篇论文不同 URL | CGO 记录 vs arXiv 预印本，13/65 来源被拒 | 标题等价判定（≥80% 词重合、最少 4 词） |
| 编号标题 | `1. Introduction` 不匹配 `Introduction` | 匹配前剥掉编号（阿拉伯/中文/括号） |
| 模型别名到期 | `expires-on-0910` 在 09-10 起被拒 | 改为 `deepseek-flash`；`max_tokens` 放在源模型上，避免 install 冲掉 |

### 当前模型与配置

```
models.deepseek / models.academic = deepseek-flash（provider: deepseek, Responses API）
  max_tokens: 32768   ← 显式输出预算；思考型模型的 reasoning 与答案共用它
  ANCHOR_MCP_AGENT_CAN_APPROVE 未设置 → agent 不能审批
学术 agent: planner / gatherer / writer / reviewer（均 output_retries: 1）
gatherer: 14 次工具（search 3 / citations 3 / read_many 3 / read 3），max_parallel_tools 6
```

模型别名带有效期时（如 `…-expires-on-0910`）会在到期日被供应商拒绝并返回替代名。
`max_tokens` 必须设在**源模型**（`models.deepseek`）上：`install` 会从源模型重建
`models.academic`，设在派生模型上会被下一次安装冲掉。

### 验收

```
run f4eb8ac7（2026-09-11）completed
  13 轮采集 · 59 来源 · 39 读过全文 · 24 组对立证据
  评审第 1 轮 major=1 → 修订；第 2 轮 major=0 → 批准
  31 分钟 · 2.21 元
  报告 48,230 字节 / 279 行 / 59 条参考文献（正文引用 59 个编号，1:1）
  正文审计：证据级标签 0、过程语言 0、内容哈希 0、最长段落 616 字符
```

成本与时间对比：每轮 155s / ¥0.48 → **110s / ¥0.10**；一次 campaign ~70 分钟 / ¥9.6 →
**~20–30 分钟 / ~¥2–3**。

### 教训

**本会话修复的 14 个缺陷里，单元测试一个都没抓到**——全部由真实运行抓出。
其中 6 个是"系统精确执行了错误的契约"，不是代码 bug。改动这类契约前先跑一遍真实场景。

另外：我连续 5 次低估成本（25x → 6x → 2.5x → 4x）。结论是**先测再说，不要估**。
