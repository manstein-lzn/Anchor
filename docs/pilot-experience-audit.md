...
状态：**P0 时的历史核查，保留作问题与决策来源，不是当前实现或待办。** 下文「未接入」「需要完成」及恢复/审批建议均指当时。P1 流式、P2 deferred 提问/审批及真实 DeepSeek HTTP/SSE 已有后续证据；2026-09-26 用户进一步撤销复杂审批扩建、跨存储事务和未知结果人工处置目标，改为保存工作记录、加载历史、由 Agent 核查后续做。当前阶段与验收以 [开发台账](pilot-development-plan.md) 为准，实现事实见 [当前架构](architecture.md)。
...
本文中的方案和排序仅保留为历史材料，不能作为当前开发指令。未确定功能现在只记录产品需求，待当前目标完成后与用户讨论；框架能力与 Anchor 已接入能力仍须分开判断。
...
当时据此提出的跨存储事务和未知结果处置前置要求已撤销；这些历史发现不自动成为当前待办。
...
原建议的「事件流 → 暂停/批准/恢复 → 上下文与计划 → 联动」顺序已失效。当前只按开发台账推进；未来产品需求不自动进入开发。
