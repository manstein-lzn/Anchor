...
> **本节记录的是那一轮当时的状态。** 下面标着「仍未做」的条目，绝大多数已在 §9 关闭；逐条核实过的
> 结果是：`tail`/`evidence` 已改为机械判定、mini 的模型配置与调用数已修、`PIPE` 已改文件+线程有界、
> 部分保存已不再称作 whole、`incomplete` 已传到模型、`skipped` 已清 `visible`、`sent` 已在拒绝之后并带
> `attempt`、overflow 重试已计数并复查 ceiling、`request_overhead` 已读真实指令与 schema、摘要是先于
> 滑窗运行的、四类摘要断言与失败留痕已补、线程收尾已修。
>
> **真正仍未关闭的只有 §9.7 列的那几条**——请以那里为准，不要以本节为准。
...
逐条核实过当前代码之后的清单，**不包含**已在 §9 关闭的项：

| # | 未关闭的 | 性质 |
| --- | --- | --- |
| 1 | **`evidence` 那两处 ABSENT 未裁定**：一次是 `budget_exhausted` 没跑完 ✓，另一次有文件但未通过规则——我**没有**判定它是真失败，还是规则不认的第三种引用写法 | 分析缺口（需人工看一眼那个文件）|
| 2 | **两臂预算不对等**：请求上限相同（各 40）✓，但 mini 侧**没有窗口** ✗——这个不对称是固有的，无法消除，只能如实声明 | 对照组属性，限制了结论强度 |
| 3 | **只验证了三个任务、每个两次**：不能从这些样本推断性能因果 ✗ | 样本量 |
| 4 | **`evidence` 的语义项机械判定覆盖不到**：来源是否真的支持引用、综合段是否引入新事实、每个文件是否真的解释了用途、清单是否至少两千行——已由 `human_review` **明确标出** ✓，但**需要人看** | 判定边界 |
| 5 | **摘要 usage 是「框架报告的」而不是「provider 自己账上的」**：记录里存的是模型响应带的 usage ✓，与 provider 账单是否逐分一致未验证 ✗ | 验证边界 |
| 6 | **共享层 patch 待主集成复核**：`sandbox.py` 与 `execenv.py` 的改动是本包提的 patch ✓，G1 冻结文件的所有权在主集成 ✓ | 流程，不在本包范围 |
| 7 | **整个第二包未获验收**：第三轮判定「仍不通过」，T1–T4 修完后**尚未有人再验** ✗ | 验收，不在本包范围 |
