...
画布默认用 Dagre 分层自动布局（没有保存坐标的图按拓扑分层，拖动过的节点坐标优先），
连线为贝塞尔曲线，回边从图下方绕行，边标签只在悬停或选中时显示。
...
顶栏“存储”视图显示整个 Anchor 的占用（数据库 + 产物）与每个图的产物占用，并允许在页面上
实时调整全局预算和每图预算；保存后立即生效，无需重启任何服务。超出预算后调度器会滚动淘汰最老的已结束运行，
“预览清理”可先看候选与受保护数量，“立即清理”需二次确认；审计记录每次清理的条数与释放字节。
预算与清理都不会终止正在运行的节点，也永不触碰等待人工、持有活跃 lease 或结果未知的运行。
